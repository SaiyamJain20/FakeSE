...
FakeSE is a full-stack, real-time virtual stock exchange built for students and educators. It lets users trade synthetic (and real) stocks with virtual money, track portfolio performance, and compete in trading leagues — all in a risk-free environment powered by a live market simulation engine.
...
FakeSE simulates a real stock exchange end-to-end:
...
```bash
git clone https://github.com/SaiyamJain20/FakeSE
```
...
```env
# Database
DATABASE_URL=postgresql://fakese:fakese@localhost:5432/fakese
...
2. Verify the Kafka topic exists:
   ```bash
   docker exec -it fakese-kafka bash -lc \
...
3. Check the backend logs for `priceTicksConsumer` errors:
   ...
   docker logs fakese-backend --tail 50
   ```
...
```bash
docker exec -it fakese-redis redis-cli
