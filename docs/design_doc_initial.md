# FakeSE — Design Document

> **Project:** FakeSE (Fake Stock Exchange)
...
- Frontend API layer now auto-attaches persisted bearer token (`fakese-token`) when an explicit token arg is not supplied, preventing accidental unauthenticated protected calls.
