...
export function OrderForm({ onOrderPlaced, initialTicker, onTickerChange, competitionId, dbTicker }: Props) {
  const prices = useMarketStore((s) => s.prices);

  // Derive initial values from initialTicker
  let initExch: "US" | "NSE" | "BSE" | "SIM" = "NSE";
  ...
  if (initialTicker) {
    ...
    else if (initialTicker === "FAKE" || (!initialTicker.includes(".") && initialTicker.length > 0)) { 
      initExch = "SIM"; 
      initSym = initialTicker; 
    }
    else { initExch = "US"; initSym = initialTicker; }
  }

  const [exchange, setExchange] = useState<"US" | "NSE" | "BSE" | "SIM">(initExch);
  ...
  const formattedTicker = formatter.format(ticker || "FAKE");
  ...
  useEffect(() => {
    if (initialTicker && initialTicker !== formattedTicker) {
      ...
      else if (initialTicker === "FAKE" || (!initialTicker.includes(".") && initialTicker.length > 0)) { setExchange("SIM"); setTicker(initialTicker); }
      else { setExchange("US"); setTicker(initialTicker); }
    }
  }, [initialTicker]); 

  useEffect(() => {
    if (onTickerChange && formattedTicker !== "FAKE" && formattedTicker !== "") {
      ...
      <form className="order-form" onSubmit={submit}>
        ...
        <label className="form-label">
          ...
          <input
            ...
            placeholder={exchange === "SIM" ? "e.g. FAKE" : exchange === "US" ? "e.g. AAPL" : "e.g. RELIANCE"}
            readOnly={!!competitionId}
            style={competitionId ? { background: "var(--surface)" } : {}}
          />
        </label>
